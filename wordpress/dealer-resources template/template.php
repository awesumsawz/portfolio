...
class dealerResources
{
  ...
  private function topicCompleteBuilder( $topic_id ): string
  {
    ...
    {
      if ($this->getTopicType($topic_id))
      {
        $output .=  "<span class=\"link-completion complete hidden\">";
        $output .=    "<iconify-icon inline icon=\"fontisto:checkbox-active\" style='color: var(--green)'></iconify-icon>";
        $output .=  "</span>";
        $output .=  "<span class=\"link-completion incomplete\">";
  ...
  private function childCardBuilder( $child_obj ): string
  {
    ...
    {
      // open it in a new tab and reveal the complete icon/hide the incomplete icon at the same time
      $output .=  "<a onclick=\"toggleDealerResourceComplete(this)\" href=\"$permalink\" target='_blank'>";
    }
    else
    {
